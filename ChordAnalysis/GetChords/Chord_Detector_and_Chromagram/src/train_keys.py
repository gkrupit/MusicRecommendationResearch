...
training_histograms = 'ChordHistograms/'
histograms_to_predict = 'ChordHistogramsToDetermine/'
...
class KeyClassifier:
    ...
    def SetTrainingData(self):
        ...
        X = [ song.Frequencies() for song in known_songs ]
        y = [ song.Key() for song in known_songs ]
    ...
    def Predict(self, filename):
        song = UnknownSong(filename)

        prediction = self.classifier.predict( [ song.Frequencies() ] )[0] # only 1 prediction
        key_value = int(prediction)
        return key_value, keys[key_value]
